 ...
 * Team 12, IoT Irrigation Controller Development
...
#define NODE_ADDR 2 // Addresses 0-255
...
void loop(void){
	...
	while (1) {
	mstr = analogRead(moisturePin);
	temp = TH02.ReadTemperature();
	rh = TH02.ReadHumidity();
	r_size = sprintf((char*)message, "M: %d%%, T: %dF, H: %d%%", mstr, temp, rh);

	PRINT_CSTSTR("%s","Sending Sensor Data"); PRINTLN;
